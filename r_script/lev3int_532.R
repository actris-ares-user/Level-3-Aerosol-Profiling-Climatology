if (flag_532) {
  ...
  for (j in 3:15) {
    db_e_y_median[, j] <- mapply(w_median_int, db_e_y_c[, j], wgs_e[, j])
    db_b_y_median[, j] <- mapply(w_median_int, db_b_y_c[, j], wgs_b[, j])
    db_e_y_sd[, j] <- mapply(w_sd_int, db_e_y_c[, j], wgs_e[, j])
  ...
  db_pd_532 <- db_pd[db_pd$Wavelength == "0532", ]
  
  ################################# Monthly averages #################################
  
  db_pd_m <- db_pd_532[, c(1, 3:4, 8:13)]
  ...
  db_pd_s <- db_pd_532[, c(1, 3:4, 8:13)]
  ...
  db_pd_nm_532 <- cbind(db_pd_nm_mean,
                       db_pd_nm_median[3:8],
                       db_pd_nm_sd[3:8],
                       db_pd_nm_cnt[3:8])
  ...
  db_pd_ns_532 <- cbind(db_pd_ns_mean,
                       db_pd_ns_median[3:8],
                       db_pd_ns_sd[3:8],
                       db_pd_ns_cnt[3:8])

bl_counts <- matrix(NA, nrow = 12, ncol = 20)
tl_counts <- matrix(NA, nrow = 12, ncol = 20)
# lr_counts <- array(NA, dim = c(12, 3, 20))  # OLD
lr_counts <- array(NA, dim = c(12, 3, 31))
# pd_counts <- array(NA, dim = c(12, 3, 20))  # OLD
pd_counts <- array(NA, dim = c(12, 3, 22)) 
aod_counts <- array(NA, dim = c(12, 3, 20))
extinction_counts <- array(NA, dim = c(12, 3, 20))
com_counts <- array(NA, dim = c(12, 3, 20))
ib_counts <- array(NA, dim = c(12, 3, 20))
backscatter_counts <- array(NA, dim = c(12, 3, 20))
...
for (p in seq_along(loc2)) {
  ...
  for (k in 1:12) {
    db_355 <- db_int_layers[db_int_layers$Station == station &
                             as.numeric(db_int_layers$Month) == k &
    ...
    db1 <- layers_base_top[layers_base_top$Station == station &
                            as.numeric(layers_base_top$Month) == k, ]
  ...
  particle_depolarization_frequency <- ncvar_def(
    ...
    missval = 9.9692099683868690e+36,
    longname = "Frequency distribution of the linear particle depolarization ratio values",
    prec = "double"
  )
  ...
  ncatt_put(month_file, longitude, "standard_name", "longitude", prec = "text")
  ncatt_put(
    month_file,
    source,
    "description",
  ...
  ncatt_put(month_file, altitude_intervals, "description", descr)
  ncatt_put(month_file, lidar_ratio_intervals, "description", descr_lr)
  ncatt_put(month_file, extinction_intervals, "description", descr)
  ncatt_put(month_file, particle_depolarization_intervals, "description", descr_pd)
  ncatt_put(month_file, aerosol_optical_depth_intervals, "description", descr)
  ncatt_put(month_file, integrated_backscatter_intervals, "description", descr)
  ncatt_put(month_file, backscatter_intervals, "description", descr)
  ncatt_put(month_file, base_layer_altitude_frequency, "histogram_intervals", "altitude_intervals")
  ncatt_put(month_file, top_layer_altitude_frequency, "histogram_intervals", "altitude_intervals")
  ncatt_put(month_file, center_of_mass_altitude_frequency, "histogram_intervals", "altitude_intervals")
  ncatt_put(month_file, lidar_ratio_frequency, "histogram_intervals", "lidar_ratio_intervals")
  ncatt_put(month_file, extinction_frequency, "histogram_intervals", "extinction_intervals")
  ncatt_put(month_file, particle_depolarization_frequency, "histogram_intervals", "particle_depolarization_intervals")
  ncatt_put(month_file, aerosol_optical_depth_frequency, "histogram_intervals", "aerosol_optical_depth_intervals")
  ncatt_put(month_file, integrated_backscatter_frequency, "histogram_intervals", "integrated_backscatter_intervals")
  ncatt_put(month_file, backscatter_frequency, "histogram_intervals", "backscatter_intervals")
  
  # Enter the values into the file
  ncvar_put(month_file, base_layer_altitude_frequency, bl_counts)
  ncvar_put(month_file, top_layer_altitude_frequency, tl_counts)
  ncvar_put(month_file, center_of_mass_altitude_frequency, com_counts)
  ncvar_put(month_file, lidar_ratio_frequency, lr_counts)
  ncvar_put(month_file, extinction_frequency, extinction_counts)
  ncvar_put(month_file, particle_depolarization_frequency, pd_counts)
  ncvar_put(month_file, aerosol_optical_depth_frequency, aod_counts)
  ncvar_put(month_file, integrated_backscatter_frequency, ib_counts)
  ncvar_put(month_file, backscatter_frequency, backscatter_counts)
  ncvar_put(month_file, altitude_intervals, altitude_intervals_data)
  ncvar_put(month_file, lidar_ratio_intervals, lr_intervals_data)
  ncvar_put(month_file, extinction_intervals, extinction_intervals_data)
  ncvar_put(month_file, particle_depolarization_intervals, pd_intervals_data)
  ncvar_put(month_file, aerosol_optical_depth_intervals, aod_intervals_data)
  ncvar_put(month_file, integrated_backscatter_intervals, ib_intervals_data)
  ncvar_put(month_file, backscatter_intervals, backscatter_intervals_data)
  ...
  ncatt_put(month_file, 
            0, 
            attname = "title", 
            attval = "Monthly distribution of layer optical values - Normal")
  nc_close(month_file)

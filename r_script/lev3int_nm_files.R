...
# Define the months
mnt <- sprintf("%02d", 1:12)

# Create the data.frame with months and a column of null values
app <- data.frame(Month = mnt, Null = NA)
...
for (p in seq_along(loc2)) {
  ...
  if (flag_355) {
    db_e355 <- filter_and_merge(db_e_nm_355, station, app)
    db_b355 <- filter_and_merge(db_b_nm_355, station, app)
    db_pd355 <- filter_and_merge(db_pd_nm_355, station, app)
  }
  
  if (flag_532) {
    db_e532 <- filter_and_merge(db_e_nm_532, station, app)
    db_b532 <- filter_and_merge(db_b_nm_532, station, app)
    db_pd532 <- filter_and_merge(db_pd_nm_532, station, app)
  }
  
  if (flag_1064) {
    db_b1064 <- filter_and_merge(db_b_nm_1064, station, app)
    db_pd1064 <- filter_and_merge(db_pd_nm_1064, station, app)
  }
  
  if (flag_pbl_nm){
    db_pbl <- filter_and_merge(db_pbl_nm_tot, station, app)
  }
  
  
  angstrom_data <- array(NA, dim = c(12, 2, 5))
  if (flag_ang_nm){
    db_ang <- filter_and_merge(db_ang_nm_tot, station, app)  
  ...
  aerosol_boundary_layer_data <- array(NA, dim = c(12, 5))
  if (flag_pbl_nm) {
  ...
  ncatt_put(month_file, integral_bounds, "flag_meaning", "0:total, 1:aerosol boundary layer", prec = "text")
  ...
  ncvar_put(month_file, aerosol_optical_depth, aerosol_optical_depth_data)
  ncvar_put(month_file, integrated_backscatter, integrated_backscatter_data)
  ncvar_put(month_file, lidar_ratio, lidar_ratio_data)
  ncvar_put(month_file, aerosol_boundary_layer, aerosol_boundary_layer_data)
  ncvar_put(month_file, h63_of_aerosol_optical_depth, h63_of_aerosol_optical_depth_data)
  ncvar_put(month_file, h63_of_integrated_backscatter, h63_of_integrated_backscatter_data)
  ncvar_put(month_file, center_of_mass, center_of_mass_data)
  ncvar_put(month_file, particle_depolarization, particle_depolarization_data)
  ...
  ncatt_put(month_file, 
            0, 
            attname = "title", 
            attval = "Monthly average integrated measurements - Normal")
  nc_close(month_file)

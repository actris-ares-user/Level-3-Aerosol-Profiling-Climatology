ssn <- list(c(1, 2, 12), 3:5, 6:8, 9:11)
...
for (p in seq_along(loc2)) {  
  ...
  for (y in release[1]:release[2]) {
    for (k in 1:4) {
      ...
      pd_vtt_532 <- as.numeric(db_532$ParicletDep[db_532$Type == "b" | db_532$PdF == 1])
    ...
    ncatt_put(season_file,
              0,
              attname = "title",
              attval = paste0("Seasonal distribution of layer optical values - year ", as.character(y)))
